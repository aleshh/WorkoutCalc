var model = {
  ...
  workoutName: function (workout) {
    switch (workout) {
      case 'sqBpDl':
        return 'Squat &middot; Bench Press &middot; Deadlift'
      case 'sqSpPc':
        return 'Squat &middot; Press &middot; Power&nbsp;Clean'
    }
  },
  ...
  nextExercise: function () {
    if (this.currentWorkout == 'sqBpDl') {
      switch (this.currentExercise) {
        case 'sq':
          this.currentExercise = 'bp'
          break
        case 'bp':
          this.currentExercise = 'dl'
          break
        case 'dl':
          this.currentExercise = 'done'
      }
    } else {
      switch (this.currentExercise) {
        case 'sq':
          this.currentExercise = 'sp'
          break
        case 'sp':
          this.currentExercise = 'pc'
          break
        case 'pc':
          this.currentExercise = 'done'
      }
  ...
  loadData: function () {
    ...
    if (!this.pastWorkouts) {
      ...
      this.lastWeights.sq.nextWeight = this.pastWorkouts[
        last
      ].exercises[0].nextWeight
      this.lastWeights.sq.lastWeight = this.pastWorkouts[
        last
      ].exercises[0].currentWeight
      ...
      while (!this.lastWeights.bp.nextWeight && i >= 0) {
        if (this.pastWorkouts[i].exercises[1].exercise == 'bp') {
          this.lastWeights.bp.nextWeight = this.pastWorkouts[
            i
          ].exercises[1].nextWeight
          this.lastWeights.bp.lastWeight = this.pastWorkouts[
            i
          ].exercises[1].currentWeight
      ...
      while (!this.lastWeights.dl.nextWeight && i >= 0) {
        if (this.pastWorkouts[i].exercises[2].exercise == 'dl') {
          this.lastWeights.dl.nextWeight = this.pastWorkouts[
            i
          ].exercises[2].nextWeight
          this.lastWeights.dl.lastWeight = this.pastWorkouts[
            i
          ].exercises[2].currentWeight
      ...
      while (!this.lastWeights.sp.nextWeight && i >= 0) {
        if (this.pastWorkouts[i].exercises[1].exercise == 'sp') {
          this.lastWeights.sp.nextWeight = this.pastWorkouts[
            i
          ].exercises[1].nextWeight
          this.lastWeights.sp.lastWeight = this.pastWorkouts[
            i
          ].exercises[1].currentWeight
      ...
      while (!this.lastWeights.pc.nextWeight && i >= 0) {
        if (this.pastWorkouts[i].exercises[2].exercise == 'pc') {
          this.lastWeights.pc.nextWeight = this.pastWorkouts[
            i
          ].exercises[2].nextWeight
          this.lastWeights.pc.lastWeight = this.pastWorkouts[
            i
          ].exercises[2].currentWeight
...
var view = {
  ...
  workoutSelectView: function () {
    ...
    if (model.lastWorkout()) {
      $('.note').html(
        'Last workout: <br><strong>' +
          model.workoutName(model.lastWorkout().workout) +
          '</strong><br>' +
  ...
  workoutView: function () {
    var setsTable = $(
      '<table id="weightRepsTable"><tr><td>Sets</td><td>Weight</td><td>Reps</td></tr></table>'
    )
    for (var i = 0; i < model.currentExerciseSets.length; i++) {
      setsTable.append(
        $(
          '<tr><td>' +
            model.currentExerciseSets[i] +
            '</td><td>' +
            model.currentExerciseWeights[i] +
            '</td><td>' +
            model.currentExerciseReps[i] +
            '</td></tr>'
        )
      )
    }
  ...
  doneWorkoutView: function () {
    ...
    t +=
      "<table><tr><td></td><td>Today's<br>Weight</td><td>Next<br>Weight</td></tr>"
  ...
  showPastWorkouts: function () {
    ...
    if (model.pastWorkouts.length == 0) {
      o =
        '<p>There are currently no workouts stored. Workouts are recorded in browser storage on your device, so you will need to use the same browser to access your data in the future.</p><p>Currently using ' +
        model.unitsLong() +
        '.</p>'
    } else {
      o = ''
      ...
      for (var i = model.pastWorkouts.length - 1; i >= 0; i--) {
        o +=
          '<p><b>' +
          model.parseDate(model.pastWorkouts[i].date) +
          '</b></p><ul>'
        for (var j = 0; j < 3; j++) {
          o +=
            '<li><b>' +
            model.exerciseName(model.pastWorkouts[i].exercises[j].exercise) +
            ':</b> '
          o +=
            model.pastWorkouts[i].exercises[j].currentWeight +
            ' ' +
            model.units() +
            ' worked<br>('
          o +=
            model.pastWorkouts[i].exercises[j].nextWeight +
            ' ' +
            model.units() +
            ' set for next time)</li>'
        }
        o += '</ul>'
      }
    ...
    v +=
      '<button type="button" class="button switch-units-button" >Switch to ' +
      model.unitsLongOpposite() +
      '</button>'
    ...
    $('.switch-units-button').click(function () {
      if (window.confirm('This will remove all stored workouts. Proceed?')) {
        ...
        $('#pastWorkouts').html(
          $('<p>Units swtiched to ' + model.unitsLong() + '.</p>')
        )
      }
    })
  },
      ...
      if (!$e.val() || $e.val() == 0) {
        ...
        if ($e.val() < model.max) {
          if ($e.val() % model.interval) {
            $e.val(
              parseInt($e.val()) + model.interval - ($e.val() % model.interval)
            )
          } else {
